...
Os controllers fazem uso de ActionResult<> e retornam objetos tipados com Ok(), CreatedAtAction(), NoContent(), BadRequest(), NotFound().
...
Ao iniciar a aplicação é criado um escopo (CreateScope) e chamado db.Database.EnsureCreated().
...
Acessar a documentação Swagger pelo navegador:
...
O CLIENTE (Postman, navegador, outra aplicação) faz uma requisição HTTP para a FutureWork API.

A requisição chega em um CONTROLLER (v1 ou v2), responsável por tratar a rota e aplicar a lógica de negócios básica.

O controller utiliza o AppDbContext (Entity Framework Core) para interagir com o banco de dados relacional (SQLite).

O EF Core executa operações de SELECT, INSERT, UPDATE, DELETE sobre o arquivo futurework.db.

A resposta retorna ao cliente em formato JSON, com o status code adequado (200, 201, 204, 400, 404, etc.).
...
Apresentação rápida do tema “Futuro do Trabalho” e da FutureWork API

Execução da API (dotnet run)

Acesso ao Swagger (/swagger)

Demonstração dos endpoints de:
...
Demonstração do endpoint de v2 com recomendações de skills

Explicação rápida do fluxo de dados (mostrar o Draw.io ou explicar na tela)

PUBLICAÇÃO EM CLOUD (OPCIONAL) ☁️
...
Este campo é opcional, mas pode ser citado no vídeo e no README para ganhar ponto extra.

RESUMO DO ATENDIMENTO AOS REQUISITOS DO PROFESSOR ✅

BOAS PRÁTICAS – API RESTful (30 pts)
...
Explicação detalhada do versionamento no README
